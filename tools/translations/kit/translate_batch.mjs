...
//   srcRepo:    "<HOME>/Documents/Codes/warp",     // where the .rs source lives
...
if (!args || !args.taskDir || !args.batchFlag || !Array.isArray(args.letters)) {
  throw new Error('translate-batch requires args = {taskDir, repoRoot, srcRepo, batchFlag, letters[]}')
}

const TASK_DIR = args.taskDir
const REPO_ROOT = args.repoRoot || '<HOME>/Documents/Codes/warp_translation'
const SRC_REPO = args.srcRepo || '<HOME>/Documents/Codes/warp'
