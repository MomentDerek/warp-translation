...
| `isolate_new.py` | Rewrite the post-extract table to `before` (pristine HEAD) + only the `status=new` additions — reverting extractor-induced fuzzy/obsolete/deleted churn so a batch + its check see a clean delta. Used by `daily-sync-translate.yml` between extract and translate. |
