#!/usr/bin/env python3
"""Apply batch-20 merged translations to translations/strings.json.

Batch-20 is a 10-way parallel merge spanning 602 entries (file-pinned).
Sub-batches A..J are composed by `research/build_candidates.py`; see
`research/batch-20-composition.md` for the file→batch mapping.
...
Idempotent: rerunning on an already-translated row is a no-op.
...
BATCH_FLAG = "pr-by-file-parallel-batch-20"
NOW = datetime.now(timezone.utc).strftime("%Y-%m-%dT%H:%M:%SZ")
BATCH_LETTERS = ["A", "B", "C", "D", "E", "F", "G", "H", "I", "J"]
EXPECTED_TOTAL = 602
